...
void sensorUCSetup() {
    ...
    P2IE |= BIT2; //Enable P1.3 interrupts
    P2IES &= ~BIT2; //Set P1.3 interrupt to rising edge
...
int removeOscillations(int currentDistance) {
    ...
    if(next >= currentDistance + bandwidth) {
        currentDistance += 1;
    } else if(next <= currentDistance - bandwidth) {
        currentDistance -= 1;
    }

    if(next >= currentDistance + 50) {
        currentDistance = next;
    } else if(next <= currentDistance - 50) {
        currentDistance = next;
    }
...
void sensorUC() {
    ...
    currentDistance = removeOscillations(currentDistance);

    switch(index){
         case 0:
             data = (index<<4) |(currentDistance)%10;//first 4 bits: index | last 4: first digit
             UCA0TXBUF = data;
             index = 1;
             break;
         case 1:
             data = (index<<4) |(currentDistance/10)%10;//first 4 bits: index | last 4: second digit
             UCA0TXBUF = data;
             index = 2;
             break;
         case 2:
             data = (index<<4) |(currentDistance/100)%10;//first 4 bits: index | last 4: third digit
             UCA0TXBUF = data;
             index = 3;
             break;
         case 3:
             data = (index<<4) |(currentDistance/1000)%10;//first 4 bits: index | last 4: fourth digit
             UCA0TXBUF = data;
             index = 0;
             break;
    }
    ...
    if(currentDistance >= 2 && currentDistance < 50) {
        TA1CCR0 = 1523;
    } else if(currentDistance >= 50 && currentDistance < 100) {
        TA1CCR0 = 1716;
    } else if(currentDistance >= 100 && currentDistance < 200) {
        TA1CCR0 = 1812;
    } else if(currentDistance >= 200 && currentDistance < 300) {
        TA1CCR0 = 2035;
    } else if(currentDistance >= 300 && currentDistance <= 400) {
        TA1CCR0 = 2289;
    }
}
